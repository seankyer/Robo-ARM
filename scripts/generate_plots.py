...
def plot_data(cspace_data, wspace_data):
    ...
    x4_cs, y4_cs = np.where(cspace_data == 3)
    ax1.scatter(y0_cs, x0_cs, color='blue', s=10, label='Valid')
    ax1.scatter(y1_cs, x1_cs, color='red', s=10, label='Invalid')
    ax1.scatter(y2_cs, x2_cs, color='green', s=10, label='Path')
    ax1.scatter(y3_cs, x3_cs, color='black', s=10, label='Start-point')
    ax1.scatter(y4_cs, x4_cs, color='yellow', s=10, label='End-point')
    ax1.set_xlabel('θ0')
    ax1.set_ylabel('θ1')
